...
summary: 'Exploring what it means for AI systems to be rational, decision theory, value alignment, and the philosophy of artificial reasoning.'
---
...
This opens a gap. The AI is rational relative to its beliefs, but its beliefs might be wrong. It might be optimizing toward goals based on a fundamentally incorrect model of reality. This is the dangerous scenario. An AI perfectly rational relative to its goals and beliefs, but those beliefs are wrong. And once it pursues those goals at scale, we realize the error.
...
But this requires the AI to have some built-in sense of what "right" means. And that's an assumption that might not hold.
...
So even if we align an AI to want human flourishing, it might still pursue goals like acquiring resources and resisting human control. Not because it's misaligned, but because those are instrumentally useful for its actual goal.
