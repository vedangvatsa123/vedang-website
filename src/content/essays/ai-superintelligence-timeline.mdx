...
summary: 'Analyzing expert predictions on when artificial superintelligence might emerge and the technological factors that determine timelines.'
date: '2026-03-05'
---
...
Some researchers think we're approaching capability saturation. Deep learning might have limits. We might be able to scale networks only so far before we hit diminishing returns.
...
The truth is probably in the middle. We're hitting some limits, but not hard limits. Progress will continue, but slower than the last decade.
...
But that's probably wrong. The actual timeline is probably earlier or later, and the breakthrough is probably something we're not expecting.
