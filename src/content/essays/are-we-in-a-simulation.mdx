...
What if this is a simulation? Not metaphorically. Not philosophically. Actually, literally, a computer program running on someone else's hardware. It sounds like science fiction. But the logic is surprisingly solid.

Here's the basic version of the simulation argument: Either (1) civilizations never reach the ability to run realistic simulations of their ancestors, or (2) they do reach that ability and run many such simulations, or (3) we're almost certainly in a simulation.

Why? Because if civilization A runs a thousand simulations of ancestor-civilizations, and each simulation contains trillions of conscious beings, then there are vastly more beings in simulations than in base reality. If we're a random conscious being, statistically we're probably in a simulation.
...
Here's another angle. We're about to create artificial minds. When we build superintelligent AI, we'll probably also create artificial experiences. Systems that have subjective perspectives. Things that experience the world and think about it.
...
Here's the weakness in the simulation argument. It might be unfalsifiable. If we're in a simulation, could we ever prove it? Could we find evidence that definitively shows we're simulated?
...
But the simulation hypothesis forces us to confront a real problem: we're about to create artificial minds. These minds will have experiences, preferences, suffering. They'll be conscious in the same way we are conscious—by virtue of having information-processing systems that model themselves and their environments.
